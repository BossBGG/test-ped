  ...
  return (
    ...
    >
      ...
      {equipmentList.map((item, index) => (
        <div
          className="flex items-center p-3 bg-[#FAF5FF] rounded-[12px] border-1 gap-3"
          key={index}
        >
          <Checkbox
            id={`equipment_${index}`}
          ...
          />
          <Label
            htmlFor={`equipment_${index}`}
            className="flex flex-col items-start w-full"
          >
            
            <div className="flex flex-row justify-between">
                {/* เลขลำดับและแสดงรายการวัสดุ */}
              <div className="font-medium w-[65%]">
                {index + 1}. {item.code || "S-3H-044"} -{" "}
                {item.name || "หม้อแปลง 3P5000KVA(รายปี)"}
              </div>

              <div className="text-sm text-gray-600 mt-1 ">
                หน่วย : {item.unit || "หน่วย"}
              </div>
            </div>
